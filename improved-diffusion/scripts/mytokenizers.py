...
def build_simple_smiles_vocab(dir):
    ...
    if not osp.exists(osp.join(dir,'simple_smiles_tokenizer_vocab.txt')):
        ...
        dirs = list(osp.join(dir,i) for i in ['train.csv','validation.csv','test.csv'])
        smiles = []
        for idir in dirs:
            ...
            for i, row in df.iterrows():
                if row['Source_Mol']!='*':
                    smiles.append(row['Source_Mol'].strip())
                if row['Target_Mol']!='*':
                    smiles.append(row['Target_Mol'].strip())
        char_set = set()
        for smi in smiles:
            for c in smi:
                char_set.add(c)
...
class regexTokenizer():
    def __init__(self,path='../../datasets/mmp/generate_vocab.txt',max_len=256):
