...
class ChEBIdataset(Dataset):
    def __init__(self,dir,smi_tokenizer,split,replace_desc=False,pre=None,prob=0,load_state=True,corrupt_prob=0.4,mask_desc=False):
        ...
        assert split in ['train','test','validation','mini','train_val', 'train_val_smiles', 'test_smiles']
    ...
    def get_ori_data(self):
        ...
        for i, row in df.iterrows():
            ...
            #                 break
            # Source_Mol_ID,Target_Mol_ID,Source_Mol,Target_Mol,Source_Mol_LogD,Target_Mol_LogD,Delta_LogD,Source_Mol_Solubility,Target_Mol_Solubility,Delta_Solubility,Source_Mol_Clint,Target_Mol_Clint,Delta_Clint,Transformation,Core,desc
            # pdb.set_trace()
            res.append((str(row['Source_Mol_ID'])+','+str(row['Target_Mol_ID']), row['Target_Mol'], row['Description']))
