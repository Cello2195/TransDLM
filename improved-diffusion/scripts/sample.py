...
def main():
    ...
    tokenizer = regexTokenizer(max_len=128)
    model = TransformerNetModel2(
        ...
        config_name='/home/data2/xyd/mol_optim/diff_optim/bert-base-uncased/config.json',
        training_mode='e2e',
    ...
    diffusion = SpacedDiffusion(
        ...
        # use_timesteps=[i for i in range(0,5000,10)],
        # betas=gd.get_named_beta_schedule('sqrt', 5000),
        model_mean_type=(
    ...
    df = pd.read_csv(f'../../datasets/mmp/{args.split}.csv')
    mols = list(df['Source_Mol'])
    ...
    while num_done < args.num_samples:
        ...
        sample_shape = (idend-num_done, 128, model.in_channels)
        print(sample_shape)
        sample = sample_fn(
            model,
            sample_shape,
...
def create_argparser():
    ...
    text_defaults = dict(modality='text',
                        ...
                        #  model_path='../../checkpoints/PLAIN_ema_0.9999_500000.pt',
                         model_path='../../checkpoints_smiles/PLAIN_ema_0.9999_500000.pt',
                         use_ddim=False,
                         batch_size = 64,
                         num_samples = 19856,
                        # num_samples = 100,
                         top_p =1.0,out_dir='generation_outputs',
                         outputdir='../../textguidtry_final_smiles.txt'
                         )
