...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        is_cross_attention = encoder_hidden_states is not None # True
        ...
        elif is_cross_attention: #  cross at here
        ...
        if attention_mask is not None:
            ...
            if len(attention_mask.shape)==2:
                attention_mask = torch.where(attention_mask==0,torch.tensor(-1e9).to(attention_mask.device),attention_mask)
                attention_mask = torch.where(attention_mask==1,torch.tensor(0.).to(attention_mask.device),attention_mask)
